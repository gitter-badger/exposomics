...
type Props = {
  ...
  startDate: string,
  endDate: string,
  mapValueToColor: any => string,
  renderTooltip: any => React$Element<any>,
  extractDate: any => Date,
};

type State = {
  data: { [key: string]: any[] },
  tooltipEntry: ?HTMLElement,
      ...
      >
        ...
        >
          {Object.keys(data).map(year => (
            <HeatmapRow
              key={year}
              onSelectEntry={e => this.selectEntry(e)}
              onMouseLeave={() => this.clearCoords()}
              text={year}
              data={data[year]}
